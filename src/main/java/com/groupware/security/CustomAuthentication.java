...
public class CustomAuthentication implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication auth) throws AuthenticationException {
		...
		String login_pwd = (String) auth.getCredentials(); //로그인 시도한 pwd를 가져온다
		...
		try {
			...
			//얘는 매니저가 받으면 바로 failure를 내보냄
			//반대로 얘가 안 나오면 로그인이 된 거라고 manager가 인식함...
		}
	...
	public boolean supports(Class<?> arg) {
		 ...
		 * default-target-url로 전송한다.
		 * 
		 * authentication이 여러가지이기 때문에 한번 확인해주는 과정임...????
		 */
